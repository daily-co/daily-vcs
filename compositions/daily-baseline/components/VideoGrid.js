...
export default function VideoGrid(gridProps) {
  ...
  return (
    <Box id="videogrid">
      {participantDescs.map((d, idx) => (
        <VideoGridItem
          key={'videogriditem_' + idx}
          index={idx}
          itemProps={d}
          gridProps={gridProps}
          totalNumItems={totalNumItems}
          itemInterval_gu={itemInterval_gu}
          outerPadding_gu={outerPadding_gu}
          preserveItemAspectRatio={preserveItemAspectRatio}
          fullScreenHighlightItemIndex={fullScreenHighlightItemIndex}
