...
if (!canvexPath || !jsonPath || !outputPath) {
  console.error("** Invalid arguments: expected 1) canvex dir, 2) json path, 3) output path");
  process.exit(1);
}
...
if (!w || !h || w < 1 || h < 1) {
  console.error("** Invalid JSON input: doesn't have valid width/height: ", jsonPath);
  process.exit(2);
}
